...
public class Prey : MonoBehaviour
{
    ...
    private float lifespan = 200f;
    private float speed = 0.1f;
    public float fieldOfView { get; private set; } = 0.8f;
    private int viewDistance = 10;
    private int maxViewDistance = 50;
    private int gestationLength = 101;
    private int offspringPerParturition = 2;
    private int executionIdx = 0;

    [SerializeField] int gestationCost = 20;
    [SerializeField] int eatingHealthBonus = 5;
    ...
    public void InitialisePrey(int executionIdx, GameManager gameManager, float[] initialisingValues)
    {
    ...
    float[] CastRays()
    {
        ...
        float[] output = new float[numberOfRays * 5];
        float angleOffset = -(float)numberOfRays * 10 / 2;

        for (int i = 0; i < numberOfRays * 5; i += 5)
        {
    ...
    void GiveBirth()
    {
        ...
        if ((currentAge >= gestationLength * (gestationCount + 1)) && (currentHealthPoints - gestationCost > 0))
        {
            for (int i = 0; i < offspringPerParturition; i++)
            {
                ...
                myGameManager.CreatePrey(spawnPosition, this.transform.rotation, this.neuralNetwork);
            }
